# mecanum_drive (Python)

Python reference implementation of the mecanum holonomic drive stack.
Functionality is identical to [`mecanum_drive_cpp`](../../cpp/mecanum_drive/README.md)
...
```bash
# Manual joystick drive (LS-X strafe, RS-X turn)
ros2 launch mecanum_drive manual_drive.launch.py

# SLAM mapping
ros2 launch mecanum_drive mapping.launch.py

# Nav2 free navigation (requires a saved map)
ros2 launch mecanum_drive navigation.launch.py map:=$HOME/maps/my_map.yaml

# Route navigation (requires a saved map + GeoJSON graph)
ros2 launch mecanum_drive route_navigation.launch.py \
...
```
DS4 → joy_node → /joy → studica_control gamepad → /cmd_vel (x, y, ω)
                                                        │
                                       mecanum_drive_node → motors (holonomic IK)

encoders + /imu → odometry_node → /odom (x, y, vx, vy, vyaw)
...
```bash
colcon build --packages-select mecanum_drive
source install/setup.bash
```

## Configuration

All config files mirror `mecanum_drive_cpp/config/` — see the
[C++ README](../../cpp/mecanum_drive/README.md) for parameter descriptions.
